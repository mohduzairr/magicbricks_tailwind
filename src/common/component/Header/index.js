    ...
    return (
        ...
        <div className="lg:w-full h-12 bg-red flex justify-between lg:items-center sm:w-full md:w-full sm:items-center">
            <div className="flex lg:ml-36 pl-12  md:ml-10 pt-3  ">
                ...
                <div onMouseEnter={()=>setCtyhide(true)} onMouseLeave={()=>setCtyhide(false)} className={`text-lg  text-white   ml-8 lg:ml-12 pt-1 font-light lg:h-12 lg:w-28 h-10 w-14 items-center ${ ctyhide ? ("bg-white text-red") : "" } pl-2 lg:pl-10 rounded`}> <span >City</span> { ctyhide==false ? <i  class="fa fa-caret-down text-white"></i> : <i  class="fa fa-caret-up text-red ctyhide==true "></i> }</div>
            </div>
            <div className="flex lg:mr-28 mr-5 pr-12  ">
                <div  onMouseEnter={()=>setLgnhide(true)} onMouseLeave={()=>setLgnhide(false)} className={`h-12 w-20 lg:mr-8 mr-4 pt-2 text-white ${ lgnhide ? ("bg-white text-red"):""} font-light text-center rounded`}> <span>Login</span> { lgnhide==false ? <i  class="fa fa-caret-down text-white"></i> : <i  class="fa fa-caret-up text-red "></i>}</div>
